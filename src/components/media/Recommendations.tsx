...
export function Recommendations({ watchedFilms, allTitles }: RecommendationsProps) {
  ...
  return (
    <>
      ...
      {/* Movie Details Modal - EXACT COPY from FilmDetailModal structure */}
      <AnimatePresence>
        {selectedRec && (
          <motion.div
            initial={{ opacity: 0 }}
          ...
          >
            ...
            >
              {/* Close Button - exact corner */}
              <button
                onClick={closeModal}
                className="absolute top-0 right-0 z-20 p-2 hover:opacity-70 transition-opacity"
              >
