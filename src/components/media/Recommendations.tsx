    ...
    <AnimatePresence>
      {selectedRec && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-50 bg-black/90 backdrop-blur-sm sm:flex sm:items-center sm:justify-center sm:p-4"
          onClick={closeModal}
        >
          <motion.div
            ...
            className="absolute inset-0 top-[env(safe-area-inset-top,0px)] sm:relative sm:inset-auto sm:top-auto sm:w-full sm:max-w-2xl sm:max-h-[90vh] sm:rounded-2xl bg-zinc-900 sm:border border-zinc-800 overflow-hidden flex flex-col"
          >
            {/* Close Button */}
            <button
              onClick={closeModal}
